...
?>
	...
	if (isset($_POST['search'])){
		...
		$myDatabase = new DatabaseConnector();	
		$movieName = $_POST['searchKeyword'];

		$array = $myDatabase->getMovie($movieName);
		...
		if (isset ($_SESSION['username']) || (!empty($_COOKIE['username']) && isset($_COOKIE['username']) )){
			...
			// echo "THIS IS USER SEARCH"; // Tet Purpose
		
			if (!empty($array)){


				$movie =  $array[0]['name'];
				$movie_ID = $array[0]['movie_ID'];

			// print_r($array); // test purpose 
				echo "<section class='container-fluid' id='search_result'>";
				echo "<div class= 'row'>";
				echo "<div class ='col-xs-12 col-sm-6  col-sm-offset-3'>";
				echo "<form action='controller.php' method = 'post'>";
				echo "<h3 class='text-left'> ". "<b> Movie: </b>". $movie."</h3>";
				
				echo "<p class='text-right'> ". "<em> Year: </em>". $array[0]['year'] ."</p>";
				echo "<p class='text-right'> ". "<em> Rank: </em>". $array[0]['rank'] ."</p>";
				// Hidden Input
				echo "<input type='hidden' class='form-control' name='movieToDownload' value=' " . $movie .  "' >" ;
				echo "<input type='hidden' class='form-control' name='movieIDToDownload' value=' " . $movie_ID .  "' >" ;
				
				
			
				
				echo "<div class='text-right'> <button id='". $movie ."' name='addToCart' type='submit' class='btn btn-primary add-to-cart text-center'> Add to the Shopping cart </button></div>";
				...
				foreach ($array as $record){	
					...
					if ($movie !=$record['name']){
						$movie = $record['name'];
						$movie_num += 1;

						echo "<p class='text-left'> ". "Actor: </p><ul class='list-group'>".  $actors  ."</ul>";
						echo "</form>";
						echo "</div>";
						echo "</div>";
						...
						echo "<div class ='col-xs-12 col-sm-6  col-sm-offset-3'>";
						echo "<form action='controller.php' method='post'>";
						echo "<h3 class='movie-year text-left'> ". "Movie: ". $record['name'] ."</h3>";
						echo "<p class='text-right'> ". "<em> Year: </em>". $record['year'] ."</p>";
						echo "<p class='text-right'> ". "<em> Rank: </em>". $record['rank'] ."</p>";
					...
					$actors = $actors. "<li class='list-group-item actor'>" .$record['first_name']	. " ". $record['last_name'] . " -------------- ". "Role: ". $record['role'] ."</li>";
				}
				echo "<p class='text-left'> ". "Actor: </p><ul class='list-group'>".  $actors  ."</ul>";
				echo "</form>";	
				echo "</div>";
			...
			} // If return an empty array, show the following message.
			else{
				echo "<h4 class='text-center'> Sorry. Opp!! Can't find ". $movieName . " in the Database</h4>";
			}
		...
		//================================ FOr GUEST Search ================================
			...
			// echo "THIS IS GUEST SEARCH"; // Tet Purpose
			...
			// print_r($array); // test purpose 
				...
				echo "THIS IS USER SEARCH";

				
			
			} // If return an empty array, show the following message.
			else{
				echo "<h4 class='text-center'> Sorry. Opp!! Can't find ". $moiveName . " in the Database</h4>";
			}
		}
			echo "</section>";
	}
	...
	<!-- Use AJAX to update the search result  -->
